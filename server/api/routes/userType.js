...
router.get("/", tokenVerification, async (req, res) => {
  try {
    ...
    const profileUpdateQuery =
      "SELECT ProfileUpdated FROM Users WHERE UserID = ?";

    const [profileUpdateRows] = await db.pool
      .promise()
      .execute(profileUpdateQuery, [userId]);

    const profileUpdated =
      profileUpdateRows[0].ProfileUpdated === 1 ? true : false;
