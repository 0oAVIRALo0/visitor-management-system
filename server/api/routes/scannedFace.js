...
router.post("/", async (req, res) => {
  try {
    ...
    if (userIdRows.length > 0) {
      ...
      const [userTypeRows] = await db.pool
        .promise()
        .execute(userTypeQuery, [userScannedFace]);
...
async function getVisitorData(userScannedFace, userId) {
  ...
  const meetingDateQuery =
    "SELECT MeetingDate FROM meetings WHERE Attendant = ?";
  const meetingTimeQuery =
    "SELECT MeetingTime FROM meetings WHERE Attendant = ?";
  const meetingLocationQuery =
    "SELECT MeetingLocation FROM meetings WHERE Attendant = ?";
  const meetingDescriptionQuery =
    "SELECT MeetingDescription FROM meetings WHERE Attendant = ?";
  const userTypeQuery = "SELECT UserType FROM Users WHERE Username = ?";

  const [userTypeRows] = await db.pool
    .promise()
    .execute(userTypeQuery, [userScannedFace]);
  const [nameRows] = await db.pool
    .promise()
    .execute(nameQuery, [userScannedFace]);
  const [emailRows] = await db.pool
    .promise()
    .execute(emailQuery, [userScannedFace]);
  const [professionRows] = await db.pool
    .promise()
    .execute(professionQuery, [userId]);
  ...
  const [meetingDateRows] = await db.pool
    .promise()
    .execute(meetingDateQuery, [userId]);
  const [meetingTimeRows] = await db.pool
    .promise()
    .execute(meetingTimeQuery, [userId]);
  const [meetingLocationRows] = await db.pool
    .promise()
    .execute(meetingLocationQuery, [userId]);
  const [meetingDescriptionRows] = await db.pool
    .promise()
    .execute(meetingDescriptionQuery, [userId]);
  const [imageRows] = await db.pool
    .promise()
    .execute(imageQuery, [userScannedFace]);

  const imageUrls = await Promise.all(
    imageRows.map((row) => getImageUrl(row.Image))
  );
...
async function getStudentData(userScannedFace, userId) {
  ...
  const [emailRows] = await db.pool
    .promise()
    .execute(emailQuery, [userScannedFace]);
  const [rollNumberRows] = await db.pool
    .promise()
    .execute(rollNumberQuery, [userId]);
